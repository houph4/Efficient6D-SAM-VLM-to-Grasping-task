...
def draw_bboxes(image, bboxes, pred_RTs, pred_scales, intrinsics, axis_length_factor=0.6):
    ...
    for i, bbox in enumerate(bboxes):
        ...
        for k in range(4):
            ...
            cv2.line(image_copy, tuple(bbox_2d[k + 4]), tuple(bbox_2d[(k + 1) % 4 + 4]), (255, 0, 0), 4)
            cv2.line(image_copy, tuple(bbox_2d[k]), tuple(bbox_2d[k + 4]), (255, 0, 0), 4)
...
def capture_video():
    ...
    input()
    print("Capturing started. Press Enter again to stop.")
    ...
    try:
        while True:
            ...
            if capture.color is not None and capture.transformed_depth is not None:
                ...
                # Visualize the RGB image while capturing
                cv2.imshow("Capturing Video", rgb_image)
                if cv2.waitKey(1) & 0xFF == ord('\r'):
                    break  # Press Enter to stop capturing
...
def preprocess_image(rgb_image, depth_image, intrinsics, area_threshold=4000):
    ...
    try:
        results = model.predict(cropped_rgb_image, conf=0.85)
    ...
    for bbox in bboxes:
        x1, y1, x2, y2 = bbox[:4]
        if(3000<abs(x1-x2)*abs(y1-y2)<8000):
            center_x = (x1 + x2) / 2
            center_y = (y1 + y2) / 2
    ...
    instances_pts_array = np.array(instances_pts)
    instances_pts = torch.FloatTensor(instances_pts_array)
    instances_rgb = torch.stack(instances_rgb)
    instances_choose_array = np.array(instances_choose)
    instances_choose = torch.LongTensor(instances_choose_array)
...
def make_frame(t, model):
    frame_index = int(t * 15)  # Assuming 30 FPS
