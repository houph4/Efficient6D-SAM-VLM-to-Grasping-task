...
class TrainingDataset(Dataset):
    def __init__(self, config, data_dir, data_type='real', num_img_per_epoch=-1, use_fill_miss=True, use_composed_img=True, per_obj=''):
        self.config = config
        self.data_dir = '/media/midea2/787d7b0f-26f9-4255-a892-756df38d1c15/home/midea1/Desktop/shapo/data'
...
class TestDataset():
    def __init__(self, config, data_dir):
        self.data_dir = '/media/midea2/787d7b0f-26f9-4255-a892-756df38d1c15/home/midea1/Desktop/shapo'
        ...
        self.intrinsics = [591.0125, 590.16775, 322.525, 244.11084]
        self.intrinsics_cam = [577.5, 577.5, 319.5, 239.5]

        result_pkl_list = glob.glob(os.path.join(self.data_dir, 'data', 'segmentation_results', 'REAL275', 'results_*.pkl'))
        result_pkl_list_cam = glob.glob(os.path.join(self.data_dir, 'data', 'segmentation_results','CAMERA25','results_*.pkl'))
        # self.result_pkl_list = sorted(result_pkl_list_cam)
    ...
    def __getitem__(self, index):
        ...
        # pts
        # cam_fx, cam_fy, cam_cx, cam_cy = self.intrinsics_cam
        cam_fx, cam_fy, cam_cx, cam_cy = self.intrinsics
        # depth = load_composed_depth(image_path) #480*640
        ...
        for j in range(num_instance):
            ...
            if len(choose)>16:
                if len(choose) <= self.sample_num:
                    ...
                    choose_idx = np.random.choice(len(choose), self.sample_num, replace=False)
                choose = choose[choose_idx]
                # visualize_choose(rgb, choose, rmin, cmin, rmax-rmin, cmax-cmin)
